...
fun scaffoldLibAction(project: Project, file: VirtualFile) = executeAction(project, file, ActionConfig(
    title = "Scaffolding library project…",
    subcommand = "scaffold",
    ...
    onBinarySuccess = { _, _, sourceFile, result, binary ->
        ...
        if (path == null) {
            ...
            try {
                val scaffoldResult = ScaffoldLib.run(sourceFile)
                openOrFocusLibraryProject(project, scaffoldResult.projectDir, "built-in")
            } catch (e: Exception) {
        ...
        openOrFocusLibraryProject(project, projectDir, "binary (${binary.name})")
    },
    onFallback = { _, _, sourceFile, _, indicator ->
        indicator.text = "Scaffolding via built-in…"
        try {
            val result = ScaffoldLib.run(sourceFile)
            openOrFocusLibraryProject(project, result.projectDir, "built-in")
        } catch (e: Exception) {
            notifyError(project, "Scaffold failed: ${e.message}")
        }
    },
...
private fun openOrFocusLibraryProject(project: Project, projectDir: File, method: String) {
    ...
    ApplicationManager.getApplication().invokeLater {
        notifyInfo(project, "Library scaffold opened using $method")
